...
if (!function_exists('convert_price_payment')) {
    function convert_price_payment($price)
    {
        return Currency::convertPricePayment($price);
    }
}
...
if (! function_exists('format_price')) {
    ...
    {
        $symbolFormat = BusinessSetting::where('type', 'symbol_format')->first();
        if ($symbolFormat && $symbolFormat->value == 1) {

            return currency_symbol() . number_format($price, BusinessSetting::where('type', 'no_of_decimals')->first()->value);
        }
        $decimals = BusinessSetting::where('type', 'no_of_decimals')->first();
        $decimalPlaces = $decimals ? $decimals->value : 2; // Default to 2 decimal places if not set

        return number_format($price, $decimalPlaces) . currency_symbol();
    }
}


if (! function_exists('format_price_payment')) {
    function format_price_payment($price)
    {
        $symbolFormat = BusinessSetting::where('type', 'symbol_format')->first();
        if ($symbolFormat && $symbolFormat->value == 1) {
...
if (! function_exists('single_price_payment')) {
    function single_price_payment($price)
    {
        return format_price_payment(convert_price_payment($price));
    }
}
...
if (! function_exists('currency_symbol')) {
    function currency_symbol()
    {
        ...
        $code = Currency::findOrFail($defaultCurrencyId)->code;
...
if (!function_exists('get_system_default_currency')) {
    ...
    {
        $default_currency_id = BusinessSetting::where('type', 'system_default_currency')->first()->value;
        return Currency::findOrFail($default_currency_id);
    }
}
...
if (!function_exists('convert_price')) {
    function convert_price($price)
    {
        ...
        //return $price; //commented by gopal
        if (Session::get('currency_exchange_rate')) {
            $price =  floatval($price) * floatval(Session::get('currency_exchange_rate'));
        } else {
            $price =  floatval($price) * floatval(get_system_default_currency()->exchange_rate);
        }
...
if (! function_exists('home_discounted_base_price_formatted')) {
    function home_discounted_base_price_formatted($id)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        ...
        return bcdiv((convertPrice($price)), 1, 2);
    }
}

//Shows Price on page based on low to high
if (! function_exists('home_price_formatted')) {
    function home_price_formatted($id)
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
...
//Shows Base Price
if (! function_exists('home_base_price_formatted')) {
    function home_base_price_formatted($id)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        if ($product->tax_type == 'percent') {
